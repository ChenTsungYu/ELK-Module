"""
(終版)
configservice doc: describe-compliance-by-resource

https://docs.aws.amazon.com/cli/latest/reference/configservice/describe-compliance-by-resource.html
https://docs.aws.amazon.com/zh_tw/config/latest/APIReference/API_DescribeComplianceByResource.html
"""
...
def main(params): 
    ...
    jData = []
    try:
        resources_list_command = f"aws configservice describe-compliance-by-resource --compliance-types COMPLIANT NON_COMPLIANT --profile {profile} --region {region} --output json"
        resources_output = subprocess.check_output(shlex.split(resources_list_command))
        jData = json.loads(resources_output)
    except Exception as e:
        pass 
    if len(jData) != 0:
        for compliance in jData['ComplianceByResources']: 
            compliance["@timestamp"] = datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")
            compliance["AccountName"] = profile
            compliance['Region'] = region
            # print(compliance)
            es.index(index='aws-config-compliance-resource', body=compliance)
